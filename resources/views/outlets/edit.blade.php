...
@section('page_title', 'Edit Outlet: ' . $outlet->name)
@section('content')
<div class="card">
    <div class="card-body">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="property_id" class="form-select @error('property_id') is-invalid @enderror" required>
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" {{ old('property_id', $outlet->property_id) == $property->id ? 'selected' : '' }}>
                                {{ $property->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="facility_template_id" class="form-select @error('facility_template_id') is-invalid @enderror" required>
                        ...
                        @foreach($facilityTemplates as $template)
                            <option value="{{ $template->id }}" {{ old('facility_template_id', $outlet->facility_template_id) == $template->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <select name="is_active" class="form-select @error('is_active') is-invalid @enderror">
                        <option value="1" {{ old('is_active', $outlet->is_active) ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ !old('is_active', $outlet->is_active) ? 'selected' : '' }}>Inactive</option>
                    </select>
