...
<div class="card card-primary card-outline shadow-lg border-0 rounded-4">
    <div class="card-body text-center p-4">
        ...
        <h1 class="h3 font-weight-bold text-dark mb-1"><?php echo e($voucher->guest_name ?? ($voucher->booking?->guest?->full_name ?? 'Temporary Guest')); ?></h1>
        <p class="text-muted small mb-4">Room: <strong><?php echo e($voucher->booking?->room_label ?? $voucher->booking?->room?->label ?? 'Temporary'); ?></strong></p>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.qr-code','data' => ['url' => $qrImageUrl,'size' => 400,'class' => 'rounded-3']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['url' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($qrImageUrl),'size' => 400,'class' => 'rounded-3']); ?>
...
<?php endif; ?>
        ...
        <div class="row text-left mb-4 bg-light p-3 rounded-3 border g-2">
            <div class="col-6">
                ...
                <span class="font-weight-bold text-dark small"><?php echo e($voucher->booking ? $voucher->booking->check_in->format('d M') . ' – ' . $voucher->booking->check_out->format('d M Y') : ($voucher->expires_at ? $voucher->expires_at->format('d M Y H:i') : 'N/A')); ?></span>
            </div>
            <div class="col-6">
                <span class="text-muted d-block small">Total Pax</span>
                <span class="font-weight-bold text-dark small">
                    <?php if($voucher->booking): ?>
                        <?php echo e($voucher->booking->total_pax + $voucher->booking->extra_beds); ?><?php echo e($voucher->addition ? ' (+' . $voucher->addition . ')' : ''); ?> guests
                    <?php else: ?>
                        <?php echo e(($voucher->pax_limit ?? 1) + ($voucher->addition ?? 0)); ?> guests
                    <?php endif; ?>
                </span>
            </div>
        </div>
...
<?php echo $__env->make('layouts.guest', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\thinkpad\Documents\Pawbxj\resort-web-qr\resources\views\vouchers\public.blade.php ENDPATH**/ ?>